...
export function useMyEnrollments(filters: EnrollmentFilters = {}) {
  return useQuery({
    queryKey: ['myEnrollments', filters],
...
export function useEnrollmentByCourse(courseId: string) {
  return useQuery({
    queryKey: ['enrollmentByCourse', courseId],
...
export function useUpdateProgress() {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['enrollmentProgress', variables.enrollmentId] });
...
export function useVideoStreamUrl(videoId: string | undefined) {
  return useQuery({
    queryKey: ['videoStreamUrl', videoId],
