...
const VARIANTS = [
  ...
  { id: 'v4', label: 'V4', name: 'Bold Fintech', href: '/design/v4' as const },
  { id: 'v5', label: 'V5', name: 'Print Editorial', href: '/design/v5' as const },
] as const;

export function DesignSwitcher() {
  const pathname = usePathname();
  const currentId = VARIANTS.find((v) => pathname.endsWith(`/${v.id}`))?.id ?? 'v1';
    ...
    >
      <div className="flex items-center gap-1 rounded-full border border-white/10 bg-neutral-900/90 p-1.5 shadow-2xl backdrop-blur-xl">
        ...
        <Link
          ...
          aria-label="Späť na prehľad variantov"
        >
